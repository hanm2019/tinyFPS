...
def main(args):
    ...
    for num_sampling in sample_list:
        ...
        with torch.no_grad():
            for j, data in tqdm(enumerate(test_data_loader), total=len(test_data_loader)):
                ...
                if args.solution == 1:  # fps
                    ...
                    batch_count_total = batch_count_total + batch_count
                elif args.solution == 13:  # random k/m batch mean divide
                    sample_points, batch_count = sampling.kdt_m_batch_mean_fps(points, num_sampling, mult_kdtree_batch, dim=False, random=True, random_divide=4)
                    batch_count_total = batch_count_total + batch_count
        ...
        if args.solution in [10, 11, 12, 13]:
